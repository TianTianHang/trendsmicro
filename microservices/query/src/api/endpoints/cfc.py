...
import torch
from core.models.cfc import CfcFit, CfcPredictor
from api.utils.create_dataset import create_sliding_window
from torch.utils.data import DataLoader,TensorDataset
import lightning as L

from api.schemas.cfc import FitData
@lru_cache
...
@router.post("/fit",response_model=FitData)
async def predict_time_interest_response(
    requeset:FitData,
):
    X = torch.tensor(requeset.timespans, dtype=torch.float32).reshape([1,-1,1])  # 添加特征维度
    y = torch.tensor(requeset.values, dtype=torch.float32).reshape([1,-1,1])
    y=y/100
    model = CfcFit(1, 128)
    trainer = L.Trainer(max_epochs=50)
    trainer.fit(model, (X,y))
    prediction=trainer.predict(model,X,return_predictions=True)
    fit_value=prediction[0].reshape(-1)*100
    return {
        "timespans":requeset.timespans,
        "values":fit_value.tolist()
    }
